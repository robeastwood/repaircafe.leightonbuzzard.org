<div>
    ...
    <div
        class="bg-white sm:rounded-xl shadow-md overflow-hidden flex flex-wrap p-4 mb-4"
    >
        ...
        <div class="basis-full sm:basis-1/2 pt-4 sm:pt-0">
            ...
            >
                ...
                <strong
                    >{{ count($event->users()->wherePivot('volunteer',true)->get()) }}</strong
                >
            </div>
            Skills available:
            {{ $event->skills()->pluck('name')->implode(', ') }}
            <div
                class="mt-2 uppercase tracking-wide text-sm text-yellow-500 font-semibold"
            >
                Guests attending:
                <strong
                    >{{ count($event->users()->wherePivot('volunteer',false)->get()) }}</strong
                >
            </div>
            <div
