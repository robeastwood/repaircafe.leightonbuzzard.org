    ...
    <x-slot name="form">
        ...
        <div class="col-span-12 sm:col-span-12">
            <p class="text-sm text-gray-600">
                We would very much appreciate your help at the Repair Cafe events, even if you're non-technical.<br />
        ...
        <div class="mb-[0.125rem] block min-h-[1.5rem] pl-[1.5rem]">
            <input wire:model.defer="volunteer"
        ...
        <div class="col-span-12 sm:col-span-12">
            <div class="mb-3">
                <select data-te-select-init multiple wire:model.defer="userSkills">
                    @foreach ($allSkills as $skill)
                        <option value="{{ $skill->id }}" {{ in_array($skill->id, $userSkills) ? 'selected' : '' }}>
                            {{ $skill->name }}</option>
                    @endforeach
                </select>
                <label data-te-select-label-ref>Please select any technical skills you have</label>
            </div>
        </div>
